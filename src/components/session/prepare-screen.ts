...
/**
 * Prepare phase screen.
 *
 * Displays a large countdown number (e.g., "10", "9", …) inside a circle,
 * with a "Get Ready" heading, brief instructional text, and a Skip action.
 *
 * The countdown value is driven by `prepareRemainingMs` in the store,
 * updated by the SessionEngine.
 */
...
TEMPLATE.innerHTML = `
  ...
  <div class="body">
    ...
    <div class="info">
      <h1>Get Ready</h1>
      <p>Find a comfortable position. Breathe naturally, or tap Skip to begin.</p>
    </div>
...
export class PrepareScreen extends HTMLElement {
  #root: ShadowRoot;
  ...
  connectedCallback() {
    ...
    // Set initial value
    this.#updateCountdown(appStore.getState().prepareRemainingMs);
  }
  ...
  #handleSkip = () => {
    this.#skipBtn?.setAttribute("disabled", "");
    sessionEngine.skipPrepare();
  };
...
customElements.define("prepare-screen", PrepareScreen);
